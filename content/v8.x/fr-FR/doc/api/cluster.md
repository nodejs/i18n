...
* {Object} 
  ...
  * `silent` {boolean} Whether or not to send output to parent's stdio. **Par défaut :** `false`.
  ...
  * `windowsHide` {boolean} Hide the forked processes console window that would normally be created on Windows systems. **Par défaut :** `false`.
