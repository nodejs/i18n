...
The `crypto` module provides the `Certificate` class for working with SPKAC data. The most common usage is handling output generated by the HTML5 `<keygen>` element. Node.js utilise [l'implémentation SPKAC d'OpenSSL](https://www.openssl.org/docs/man1.0.2/apps/spkac.html) en interne.
...
Les instances de la classe `Cipher` sont utilisées pour crypter des données. The class can be used in one of two ways:
...
- Renvoie : {Buffer | string} Tout contenu chiffré restant. If `outputEncoding` parameter is one of `'latin1'`, `'base64'` or `'hex'`, a string is returned. Si `outputEncoding` est omis, un [`Buffer`][] est renvoyé.
...
Les instances de la classe `Decipher` sont utilisées pour déchiffre des données. The class can be used in one of two ways:
...
- `outputEncoding` {string}
- Renvoie : {Buffer | string} Tout contenu déchiffré restant. If `outputEncoding` parameter is one of `'latin1'`, `'ascii'` or `'utf8'`, a string is returned. Si `outputEncoding` est omis, un [`Buffer`][] est renvoyé.
...
L'`encoding` peut être `'latin1'`, `'hex'` ou `'base64'`. If `encoding` is provided a string is returned; otherwise a [`Buffer`][] is returned.
...
L'`encoding` peut être `'latin1'`, `'hex'` ou `'base64'`. If `encoding` is specified, a string is returned; otherwise a [`Buffer`][] is returned.
...
Définit la clé privée de Courbe Elliptique Diffie-Hellman. The `encoding` can be `'latin1'`, `'hex'` or `'base64'`. If `encoding` is provided, `privateKey` is expected to be a string; otherwise `privateKey` is expected to be a [`Buffer`][], `TypedArray`, or `DataView`.
...
Définit la clé publique de Courbe Elliptique Diffie-Hellman. Key encoding can be `'latin1'`, `'hex'` or `'base64'`. If `encoding` is provided `publicKey` is expected to be a string; otherwise a [`Buffer`][], `TypedArray`, or `DataView` is expected.
...
La classe `Hash` est un utilitaire pour créer des condensés de données de hachage. It can be used in one of two ways:
...
La méthode [`crypto.createHash()`][] est utilisée pour créer les instances de `Hash`. `Hash` objects are not to be created directly using the `new` keyword.
...
La classe `Hmac` est un utilitaire pour la création de condensés cryptographiques HMAC. It can be used in one of two ways:
...
La méthode [`crypto.createHmac()`][] est utilisée pour créer les instances de `Hash`. `Hmac` objects are not to be created directly using the `new` keyword.
...
Calcule le condensé HMAC de toutes les données passées via [`hmac.update()`][]. L'`encoding` peut être `'hex'`, `'latin1'` ou `'base64'`. If `encoding` is provided a string is returned; otherwise a [`Buffer`][] is returned;

The `Hmac` object can not be used again after `hmac.digest()` has been called. Plusieurs appels à `hmac.digest()` génèreront une erreur.
...
La classe `Sign` est un utilitaire pour générer des signatures. It can be used in one of two ways:
...
La méthode [`crypto.createSign()`][] est utilisée pour créer les instances de `Sign`. The argument is the string name of the hash function to use. `Sign` objects are not to be created directly using the `new` keyword.
...
In some cases, a `Sign` instance can also be created by passing in a signature algorithm name, such as 'RSA-SHA256'. This will use the corresponding digest algorithm. This does not work for all signature algorithms, such as 'ecdsa-with-SHA256'. Utilisez les noms abrégés à la place.
...
L'argument `privateKey` peut être un objet ou une chaîne. If `privateKey` is a string, it is treated as a raw key with no passphrase. If `privateKey` is an object, it must contain one or more of the following properties:
...
The `Sign` object can not be again used after `sign.sign()` method has been called. Plusieurs appels à `sign.sign()` génèreront une erreur.
...
La classe `Verify` est un utilitaire pour vérifier des signatures. It can be used in one of two ways:
...
Creates an Elliptic Curve Diffie-Hellman (`ECDH`) key exchange object using a predefined curve specified by the `curveName` string. Utilisez [`crypto.getCurves()`][] pour obtenir une liste des noms de courbes disponibles. On recent OpenSSL releases, `openssl ecparam -list_curves` will also display the name and description of each available elliptic curve.
