...
Une chaîne de caractères URL est une chaîne structurée contenant plusieurs composants significatifs. When parsed, a URL object is returned containing properties for each of these components.
...
Une comparaison entre les APIs WHATWG et celle héritée est offerte ci-dessous. Above the URL `'http://user:pass@sub.host.com:8080/p/a/t/h?query=string#hash'`, properties of an object returned by the legacy `url.parse()` are shown. Below it are properties of a WHATWG `URL` object.
...
Browser-compatible `URL` class, implemented by following the WHATWG URL Standard. [Des exemples des URLs analysés](https://url.spec.whatwg.org/#example-url-parsing) se trouvent dans le Standard lui-même.
...
Crée un nouvel objet `URL` en analysant l'`input` relatif à la `base`. If `base` is passed as a string, it will be parsed equivalent to `new URL(base)`.
...
Une `TypeError` sera lancée si l'`input` ou `base` ne sont pas d'URL valide. Note that an effort will be made to coerce the given values into strings. For instance:
...
*Note*: This feature is only available if the `node` executable was compiled with [ICU](intl.html#intl_options_for_building_node_js) enabled. Sinon, les noms de domaine y sont passés inchangés.
...
* `options` {Object} 
  * `auth` {boolean} `true` if the serialized URL string should include the username and password, `false` otherwise. **Par défaut :** `true`.
  * `fragment` {boolean} `true` if the serialized URL string should include the fragment, `false` otherwise. **Par défaut :** `true`.
  * `search` {boolean} `true` if the serialized URL string should include the search query, `false` otherwise. **Par défaut :** `true`.
  * `unicode` {boolean} `true` if Unicode characters appearing in the host component of the URL string should be encoded directly as opposed to being Punycode encoded. **Par défaut :** `false`.
...
* `parseQueryString` {boolean} If `true`, the `query` property will always be set to an object returned by the [`querystring`][] module's `parse()` method. If `false`, the `query` property on the returned URL object will be an unparsed, undecoded string. **Par défaut :** `false`.
* `slashesDenoteHost` {boolean} If `true`, the first token after the literal string `//` and preceding the next `/` will be interpreted as the `host`. For instance, given `//foo/bar`, the result would be `{host: 'foo', pathname: '/bar'}` rather than `{pathname: '//foo/bar'}`. **Par défaut :** `false`.
