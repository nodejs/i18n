...
*Note*: While the Legacy API has not been deprecated, it is maintained solely for backwards compatibility with existing applications. Le codage d'application nouvelle devrait emprunter l'API WHATWG.
...
La classe `URL`, compatible avec les navigateurs, est implémentée par le Standard URL WHATWG suivant. [Des exemples des URLs analysés](https://url.spec.whatwg.org/#example-url-parsing) se trouvent dans le Standard lui-même.
...
Les caractères URL invalides inclus dans la valeur assignée à la propriété `mot de passe` sont [percent-encoded](#whatwg-percent-encoding). Note que la séléction de caractères à pourcent-encoder peuvent varier un peu par rapport à ce que produit le méthodes [`url.parse()`][] et [`url.format()`][].
