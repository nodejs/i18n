...
* `options` {Object}
  * `showHidden` {boolean} If `true`, the `object`'s non-enumerable symbols and properties will be included in the formatted result. **По умолчанию:** `false`.
  * `depth` {number} Specifies the number of times to recurse while formatting the `object`. Полезно для проверки больших сложных объектов. По умолчанию: `2`. To make it recurse indefinitely pass `null`.
...
added: v0.3.0
deprecated: v0.11.3
-->> Стабильность: 0 - устарело: вместо этого используйте [`console.error()`] [].
...
added: v0.3.0
deprecated: v0.11.3
-->> Стабильность: 0 - устарело: вместо этого используйте [`console.log()`] [].
