...
* The `server.listen()` method may be called multiple times. Каждый последующий вызов *повторно откроет* сервер, используя предоставленные опции.
...
Set the encoding for the socket as a [Readable Stream](stream.html#stream_class_stream_readable). За подробностями обращайтесь к [`stream.setEncoding()`][].
