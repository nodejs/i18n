...
Метод запроса в качестве строки. Read-only. Пример: `'GET'`, `'DELETE'`.
...
Ответ реализует, но не наследует, интерфейс [Writable Stream](stream.html#stream_writable_streams). Это [`EventEmitter`][] со следующими событиями:
