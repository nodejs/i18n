...
* {number} Timeout in milliseconds. **Προεπιλογή:** `120000` (2 λεπτά).
...
* {number} Timeout in milliseconds. **Προεπιλογή:** `5000` (5 δευτερόλεπτα).
...
* `options` {Object | string | URL} 
  ...
  * `port` {number} Port of remote server. **Προεπιλογή:** `80`.
  * `localAddress` {string} Local interface to bind for network connections.
  * `socketPath` {string} Unix Domain Socket (use one of host:port or socketPath).
  * `method` {string} A string specifying the HTTP request method. **Default:** `'GET'`.
  * `path` {string} Request path. Θα πρέπει να συμπεριλαμβάνει το string ερωτήματος, αν υπάρχει. E.G. `'/index.html?page=12'`. An exception is thrown when the request path contains illegal characters. Currently, only spaces are rejected but that may change in the future. **Προεπιλογή:** `'/'`.
