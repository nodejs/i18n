...
**在 Node.js 中绑定的 punycode 模块版本正在被弃用**。 在未来的主要版本的 Node.js 中，此模块将被删除。 Users currently depending on the `punycode` module should switch to using the userland-provided [Punycode.js](https://mths.be/punycode) module instead.

`punycode` 模块是打包后的 [Punycode.js](https://mths.be/punycode) 模块。 It can be accessed using:
