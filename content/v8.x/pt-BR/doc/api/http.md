...
-->* `opções` {Object | string | URL} aceita as mesmas `Opções` que [] [`http.request()`], com o `método` sempre definido para `GET`. Properties that are inherited from the prototype are ignored.
* `callback` {Function}
