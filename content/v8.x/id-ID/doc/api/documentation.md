...
Kompatibilitas terbalik antar versi utama tidak diharapkan.
```
...
Beberapa panggilan sistem, seperti Ichown(2), spesifik untuk BSD. That means, for example, that [`fs.lchown()`][] only works on macOS and other BSD-derived systems, and is not available on Linux.
