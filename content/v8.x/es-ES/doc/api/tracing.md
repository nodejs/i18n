...
The set of categories for which traces are recorded can be specified using the `--trace-event-categories` flag followed by a list of comma separated category names. Por defecto, se habilitan las categorías `node`, `node.async_hooks`, y `v8` .
