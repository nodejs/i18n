...
The `crypto` module provides the `Certificate` class for working with SPKAC data. The most common usage is handling output generated by the HTML5 `<keygen>` element. Node.js usa [OpenSSL's SPKAC implementation](https://www.openssl.org/docs/man1.0.2/apps/spkac.html) internamente.
...
Instancias de clase `Cipher` son usadas para encriptar datos. The class can be used in one of two ways:
...
- Devuelve: {Buffer | string} Cualquier contenido cifrado restante. If `outputEncoding` parameter is one of `'latin1'`, `'base64'` or `'hex'`, a string is returned. Si no se provee una `outputEncoding`, un [`Buffer`][] es devuelto.
...
Instancias de la clase `Decipher` son usadas para descifrar datos. The class can be used in one of two ways:
...
- `outputEncoding` {string}
- Devuelve: {Buffer | string} Cualquier contenido descifrado restante. If `outputEncoding` parameter is one of `'latin1'`, `'ascii'` or `'utf8'`, a string is returned. Si no se provee una `outputEncoding`, un [`Buffer`][] es devuelto.
...
El argumento `encoding` puede ser `'latin1'`, `'hex'`, o `'base64'`. If `encoding` is provided a string is returned; otherwise a [`Buffer`][] is returned.
...
El argumento `encoding` puede ser `'latin1'`, `'hex'`, o `'base64'`. If `encoding` is specified, a string is returned; otherwise a [`Buffer`][] is returned.
...
Establece la clave privada EC Diffie-Hellman. The `encoding` can be `'latin1'`, `'hex'` or `'base64'`. If `encoding` is provided, `privateKey` is expected to be a string; otherwise `privateKey` is expected to be a [`Buffer`][], `TypedArray`, or `DataView`.
...
Establece la clave pública EC Diffie-Hellman. Key encoding can be `'latin1'`, `'hex'` or `'base64'`. If `encoding` is provided `publicKey` is expected to be a string; otherwise a [`Buffer`][], `TypedArray`, or `DataView` is expected.
...
La clase `Hash` es una utilidad para crear resúmenes hash de datos. It can be used in one of two ways:
...
El método [`crypto.createHash()`][] es usado para crear las instancias `Hash`. `Hash` objects are not to be created directly using the `new` keyword.
...
Calculates the digest of all of the data passed to be hashed (using the [`hash.update()`][] method). El `encoding` puede ser `'hex'`, `'latin1'` o `'base64'`. If `encoding` is provided a string will be returned; otherwise a [`Buffer`][] is returned.

The `Hash` object can not be used again after `hash.digest()` method has been called. Llamados múltiples causaran que se arroje un error.
...
La clase `Hmac` es una utilidad para crear resúmenes criptográficos HMAC. It can be used in one of two ways:
...
El método [`crypto.createHmac()`][] es usado para crear instancias `Hmac`. `Hmac` objects are not to be created directly using the `new` keyword.
...
Calcula el resúmen HMAC de todos los datos pasados usando [`hmac.update()`][]. El `encoding` puede ser `'hex'`, `'latin1'` o `'base64'`. If `encoding` is provided a string is returned; otherwise a [`Buffer`][] is returned;

The `Hmac` object can not be used again after `hmac.digest()` has been called. Llamadas múltiples a `hmac.digest()` producirá un error.
...
La clase `Sign` es una utilidad para generar firmas. It can be used in one of two ways:
...
El método [`crypto.createSign()`][] es usado para crear instancias `Sign`. The argument is the string name of the hash function to use. `Sign` objects are not to be created directly using the `new` keyword.
...
In some cases, a `Sign` instance can also be created by passing in a signature algorithm name, such as 'RSA-SHA256'. This will use the corresponding digest algorithm. This does not work for all signature algorithms, such as 'ecdsa-with-SHA256'. En su lugar, use nombres resumidos.
...
El argumento `privateKey` puede ser un objeto o una string. If `privateKey` is a string, it is treated as a raw key with no passphrase. If `privateKey` is an object, it must contain one or more of the following properties:
...
The `Sign` object can not be again used after `sign.sign()` method has been called. Llamadas múltiples a `sign.sign()` van a resultar en un error.
...
La clase `Verify` es una utilidad para verificar firmas. It can be used in one of two ways:
...
Verifica los datos dados usando los `object` y `signature` dados. The `object` argument can be either a string containing a PEM encoded object, which can be an RSA public key, a DSA public key, or an X.509 certificate, or an object with one or more of the following properties:
...
Las nuevas aplicaciones deben esperar que el valor por defecto sea `'buffer'`. This property may become deprecated in a future Node.js release.
...
Creates and returns a `Cipher` object that uses the given `algorithm` and `password`. El argumento opcional `options` controla el comportamiento del stream.

El `algorithm` es dependiente de OpenSSL, un ejemplo sería `'aes192'`, etc. On recent OpenSSL releases, `openssl list-cipher-algorithms` will display the available cipher algorithms.
...
Creates and returns a `Cipher` object, with the given `algorithm`, `key` and initialization vector (`iv`). El argumento opcional `options` controla el comportamiento del stream.
...
The `crypto.createCredentials()` method is a deprecated function for creating and returning a `tls.SecureContext`. No debe ser usada. Replace it with [`tls.createSecureContext()`][] which has the exact same arguments and return value.
...
Creates and returns a `Decipher` object that uses the given `algorithm` and `password` (key). El argumento opcional `options` controla el comportamiento del stream.
...
Creates and returns a `Decipher` object that uses the given `algorithm`, `key` and initialization vector (`iv`). El argumento opcional `options` controla el comportamiento del stream.
...
Creates an Elliptic Curve Diffie-Hellman (`ECDH`) key exchange object using a predefined curve specified by the `curveName` string. Use [`crypto.getCurves()`][] para obtener una lista de nombres de curvas disponibles. On recent OpenSSL releases, `openssl ecparam -list_curves` will also display the name and description of each available elliptic curve.
...
Creates and returns a `Hash` object that can be used to generate hash digests using the given `algorithm`. El argumento opcional `options` controla el comportamiento del stream.

The `algorithm` is dependent on the available algorithms supported by the version of OpenSSL on the platform. Los ejemplos son `'sha256'`, `'sha512'`, etc. On recent releases of OpenSSL, `openssl list-message-digest-algorithms` will display the available digest algorithms.
...
Crea y devuelve un objeto `Sign` que usa el `algorithm` dado. Use [`crypto.getHashes()`][] to obtain an array of names of the available signing algorithms. Optional `options` argument controls the `stream.Writable` behavior.
...
Crea y devuelve un objeto `Verify` que usa el algoritmo dado. Use [`crypto.getHashes()`][] to obtain an array of names of the available signing algorithms. Optional `options` argument controls the `stream.Writable` behavior.
...
The supplied `callback` function is called with two arguments: `err` and `derivedKey`. If an error occurs, `err` will be set; otherwise `err` will be null. La `derivedKey` exitosamente generada pasará como un [`Buffer`][].
...
La `salt` también debe ser lo más única posible. It is recommended that the salts are random and their lengths are at least 16 bytes. See [NIST SP 800-132](http://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-132.pdf) for details.
...
`privateKey` puede ser un objeto o una string. If `privateKey` is a string, it is treated as the key with no passphrase and will use `RSA_PKCS1_OAEP_PADDING`.
...
`key` puede ser un objeto o una string. If `key` is a string, it is treated as the key with no passphrase and will use `RSA_PKCS1_PADDING`.
...
Genera datos pseudoaleatorios criptográficamente fuertes. The `size` argument is a number indicating the number of bytes to generate.

If a `callback` function is provided, the bytes are generated asynchronously and the `callback` function is invoked with two arguments: `err` and `buf`. Si un error ocurre, `err` será un objeto de Error, de lo contrario sera nulo. The `buf` argument is a [`Buffer`][] containing the generated bytes.
...
The `crypto.randomBytes()` method will not complete until there is sufficient entropy available. Esto normalmente no debe tardar más de unos pocos milisegundos. The only time when generating the random bytes may conceivably block for a longer period of time is right after boot, when the whole system is still low on entropy.
...
Esta función se basa en un algoritmo de tiempo constante. Returns true if `a` is equal to `b`, without leaking timing information that would allow an attacker to guess one of the values. This is suitable for comparing HMAC digests or secret values like authentication cookies or [capability urls](https://www.w3.org/TR/capability-urls/).
...
El uso de `ECDH` sin dinamismo generado de pares key ha sido simplificado. Now, [`ecdh.setPrivateKey()`][] can be called with a preselected private key and the associated public point (key) will be computed and stored in the object. Esto permite al código sólo almacenar y proporcionar la parte privada del par key EC. [`ecdh.setPrivateKey()`][] now also validates that the private key is valid for the selected curve.
