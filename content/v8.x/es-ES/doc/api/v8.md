...
`v8.setFlagsFromString()` method can be used to programmatically set V8 command line flags. Este método debería ser utilizado con cuidado. Changing settings after the VM has started may result in unpredictable behavior, including crashes and data loss; or it may simply do nothing.
...
Marca a un `ArrayBuffer` que tiene sus contenidos transferidos fuera de banda. Pass the corresponding `ArrayBuffer` in the deserializing context to [`deserializer.transferArrayBuffer()`][].
