...
Per la maggior parte delle funzioni del modulo `fs`, gli argomenti `path` o `filename` possono essere passati come [`URL`][] object WHATWG. Sono supportati solo gli [`URL`][] object che utilizzano il protocollo `file:`.
...
L'argomento `filename` potrebbe non essere fornito a seconda del supporto del sistema operativo. If `filename` is provided, it will be provided as a `Buffer` if `fs.watch()` is called with its `encoding` option set to `'buffer'`, otherwise `filename` will be a string.
...
Prior to Node v0.12, the `ctime` held the `birthtime` on Windows systems. Da notare che a partire dalla v0.12, `ctime` non è "creation time" e sui sistemi Unix non lo è mai stato.
