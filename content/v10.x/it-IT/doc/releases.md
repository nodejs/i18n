...
Release builds require manual promotion by an individual with SSH access to the server as the _dist_ user. Il [Node.js build team](https://github.com/nodejs/build) è in grado di fornire questo accesso ai singoli autorizzati dal TSC.

### 3. Una GPG Key elencata pubblicamente

Un file SHASUMS256.txt è prodotto per ogni build promossa, ogni notte, e release. Inoltre per le release, questo file è firmato dall'individuo responsabile di quella versione. Per poter verificare i binari scaricati, il pubblico dovrebbe essere in grado di controllare che il file SHASUMS256.txt sia stato firmato da qualcuno che è stato autorizzato a creare una release.

Le chiavi GPG devono essere scaricabili da un keyserver di terze parti noto. The SKS Keyservers at <https://sks-keyservers.net> are recommended. Usa il modulo [submission](https://pgp.mit.edu/) per inviare una nuova chiave GPG. Le chiavi devono essere recuperabili tramite:
...
Inoltre, le impronte digitali delle chiavi GPG complete per le persone autorizzate a rilasciare dovrebbero essere elencate nel file README.md di Node.js GitHub.

## Come creare una release

Note:

- Dates listed below as _"YYYY-MM-DD"_ should be the date of the release **as UTC**. Usa `data -u +'%Y-%m-%d'` per scoprire di cosa si tratta.
