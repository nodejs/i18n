...
* For each abandoned V8 branch corresponding to an LTS branch that is affected by the bug: 
  ...
  * Su Node.js < 9.0.0: Aumenta la versione a livello di patch in `v8-version.h`. This will not cause any problems with versioning because V8 will not publish other patches for this branch, so Node.js can effectively bump the patch version.
