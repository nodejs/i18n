...
Per impedire l'arresto anomalo del processo di Node.js è possibile utilizzare il modulo [`domain`][]. (Note, however, that the `domain` module is deprecated.)
...
Restituisce un array che elenca gli eventi per i quali l'emitter ha registrato i listener. The values in the array will be strings or `Symbol`s.
