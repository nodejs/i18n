...
`cluster.on('disconnect')` event와 비슷하지만, 해당 worker에만 적용됩니다.
...
`cluster.on('exit')` event와 비슷하지만, 해당 worker에만 적용됩니다.
...
`cluster.on('listening')` event와 비슷하지만, 해당 worker 오브젝트에만 적용됩니다 .
