...
The `crypto` module provides the `Certificate` class for working with SPKAC data. The most common usage is handling output generated by the HTML5 `<keygen>` element. Node.js utilise [l'implémentation SPKAC d'OpenSSL](https://www.openssl.org/docs/man1.1.0/apps/openssl-spkac.html) en interne.
...
Calcule le condensé HMAC de toutes les données passées via [`hmac.update()`][]. If `encoding` is provided a string is returned; otherwise a [`Buffer`][] is returned;
