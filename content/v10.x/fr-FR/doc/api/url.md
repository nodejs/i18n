...
Une comparaison entre les APIs WHATWG et celle héritée est offerte ci-dessous. Above the URL `'http://user:pass@sub.example.com:8080/p/a/t/h?query=string#hash'`, properties of an object returned by the legacy `url.parse()` are shown. Below it are properties of a WHATWG `URL` object.
...
Browser-compatible `URL` class, implemented by following the WHATWG URL Standard. [Des exemples des URLs analysés](https://url.spec.whatwg.org/#example-url-parsing) se trouvent dans le Standard lui-même. La classe `URL` est aussi disponislbe sur l'objet global.
