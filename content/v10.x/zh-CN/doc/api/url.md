...
对于 WHATWG 和旧版本 API的比较如下。 Above the URL `'http://user:pass@sub.example.com:8080/p/a/t/h?query=string#hash'`, properties of an object returned by the legacy `url.parse()` are shown. Below it are properties of a WHATWG `URL` object.
...
Browser-compatible `URL` class, implemented by following the WHATWG URL Standard. [解析过的 URL 范例](https://url.spec.whatwg.org/#example-url-parsing) 可以在标准本身中找到。 The `URL` class is also available on the global object.
