...
The `repl` module provides a Read-Eval-Print-Loop (REPL) implementation that is available both as a standalone program or includible in other applications. 可以通过如下方式访问：
...
`_` (underscore). 显式设置 `_` 为某个值将禁用此行为。
