...
Μια απόσυρση μόνο στην τεκμηρίωση, είναι αυτή που εκφράζεται μόνο μέσω της τεκμηρίωσης του API της Node.js. Αυτές δεν έχουν καμία επίδραση στην εκτέλεση της Node.js. Κάποιες αποσύρσεις μόνο στην τεκμηρίωση, ενεργοποιούν μια προειδοποίηση κατά την εκτέλεση, όταν γίνεται εκκίνηση με την παράμετρο [`--pending-deprecation`][] (ή την εναλλακτική μεταβλητή περιβάλλοντος, `NODE_PENDING_DEPRECATION=1`), όπως περιγράφεται παρακάτω για τις αποσύρσεις κατά την εκτέλεση. Οι αποσύρσεις μόνο στην τεκμηρίωση που υποστηρίζουν αυτήν την παράμετρο, επισημαίνονται ρητά στην [λίστα των αποσυρόμενων API](#deprecations_list_of_deprecated_apis).
...
Περιστασιακά, η απόσυρση ενός API μπορεί να ανακληθεί. Σε αυτές τις περιπτώσεις, αυτό το έγγραφο θα ενημερωθεί με πληροφορίες σχετικές με την απόφαση. Ωστόσο, το αναγνωριστικό απόσυρσης δεν θα τροποποιηθεί.
...
* [`Buffer.from(string[, encoding])`](buffer.html#buffer_class_method_buffer_from_string_encoding) - Δημιουργία `Buffer` που αντιγράφει το `string`.

Από την έκδοση v10.0.0, μια προειδοποίηση απόσυρσης εμφανίζεται κατά την εκτέλεση όταν χρησιμοποιείται το `--pending-deprecation` ή όταν ο κώδικας κλήσης βρίσκεται εκτός `node_modules` με σκοπό την ορθότερη στόχευση των προγραμματιστών, και όχι των χρηστών.
...
### DEP0007: Αντικατάσταση του cluster worker.suicide με το worker.exitedAfterDisconnect
<!-- YAML
changes:
  - version: v9.0.0
...
Σε μια προηγούμενη έκδοση του Node.js `cluster`, μια ιδιότητα boolean με το όνομα `suicide` προστέθηκε στο αντικείμενο `Worker`. Ο σκοπός αυτής της ιδιότητας ήταν να παρέχει μια ένδειξη για τον τρόπο και την αιτία που τερματίστηκε το στιγμιότυπο του `Worker`. Στη Node.js έκδοση 6.0.0, η παλιά ιδιότητα αποσύρθηκε και αντικαταστάθηκε με τη νέα ιδιότητα [`worker.exitedAfterDisconnect`][]. Το παλιό όνομα της ιδιότητας δεν περιέγραφε την ακριβή σημασιολογία και ήταν άσκοπα συναισθηματικά φορτισμένη.
...
The `constants` module is deprecated. Όταν απαιτείται πρόσβαση στις σταθερές συγκεκριμένων ενοτήτων της Node.js, θα πρέπει να ανατρέχουν στην ιδιότητα `constants` που διαθέτει η κάθε ενότητα. Για παράδειγμα, `require('fs').constants` και `require('os').constants`.
