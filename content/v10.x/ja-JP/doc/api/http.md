...
The HTTP interfaces in Node.js are designed to support many features of the protocol which have been traditionally difficult to use. 特に、大規模で、ひとまとめにエンコードされたメッセージに向いています。 The interface is careful to never buffer entire requests or responses — the user is able to stream data.
