...
* Interne Node.js bibliotheken. Node.js itself exports a number of C++ APIs that Addons can use &mdash; the most important of which is the `node::ObjectWrap` class.

* Node.js includes a number of other statically linked libraries including OpenSSL. These other libraries are located in the `deps/` directory in the Node.js source tree. Only the libuv, OpenSSL, V8 and zlib symbols are purposefully re-exported by Node.js and may be used to various extents by Addons. Zie [Link naar eigen afhankelijkheden van Node.js](#addons_linking_to_node_js_own_dependencies) voor meer informatie.
...
Next, invoke the `node-gyp build` command to generate the compiled `addon.node` file. Dit wordt dan in de `build/Release/` map gezet.
...
Pas de verschillende methoden die onthult moeten worden toe in `myobject.cc`. Below, the method `plusOne()` is exposed by adding it to the constructor's prototype:
