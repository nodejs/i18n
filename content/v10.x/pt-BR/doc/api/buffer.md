...
Antes da introdução de [`TypedArray`], a língua JavaScript não tinha um mecanismo para leitura ou manipulação de fluxos de dados binários. The `Buffer` class was introduced as part of the Node.js API to enable interaction with octet streams in TCP streams, file system operations, and other contexts.
