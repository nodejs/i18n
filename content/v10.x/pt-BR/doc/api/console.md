...
Cria um novo `Console` com uma ou duas instâncias de fluxo gravável. `stdout` is a writable stream to print log or info output. `stderr` is used for warning or error output. Se não for fornecido `stderr`, `stdout` é usado para `stderr`.
...
The global `console` is a special `Console` whose output is sent to [`process.stdout`][] and [`process.stderr`][]. É equivalente a chamar:
