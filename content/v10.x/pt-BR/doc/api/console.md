...
***Warning***: The global console object's methods are neither consistently synchronous like the browser APIs they resemble, nor are they consistently asynchronous like all other Node.js streams. Veja a [nota de processo I/O](process.html#process_a_note_on_process_i_o) para mais Informações.
...
* `options` {Object}
  ...
  * `colorMode` {boolean|string} conjunto suporte a cores para esta instância do `Console`. Ajuste para `true` permite colorir enquanto inspecionando valores, configuração para `'auto'` fará suporte cor dependem do valor da propriedade `isTTY` e o valor retornado pelo `getColorDepth()` no respectivo fluxo. **Default:** `'auto'`.
