...
* **child_process**: `spawn()` y `spawnSync()` ahora soporta una opción `'shell'` para permitir ejecución opcional del comando dado dentro de un shell. If set to `true`, `cmd.exe` will be used on Windows and `/bin/sh` elsewhere. A path to a custom shell can also be passed to override these defaults. On Windows, this option allows `.bat.` and `.cmd` files to be executed with `spawn()` and `spawnSync()`. (Colin Ihrig) [#4598](https://github.com/nodejs/node/pull/4598)
