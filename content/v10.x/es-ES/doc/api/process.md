...
Las funciones del oyente sólo **deben** realizar operaciones **sincrónicas**. The Node.js process will exit immediately after calling the `'exit'` event listeners causing any additional work still queued in the event loop to be abandoned. En el siguiente ejemplo, el timeout (tiempo de espera) nunca ocurrirá:
...
El método `process.setuid(id)` establece la identidad del usuario del proceso. (See setuid(2).) The `id` can be passed as either a numeric ID or a username string. If a username is specified, the method blocks while resolving the associated numeric ID.
...
The return value includes fractions of a second. Utilice `Math.floor()` para obtener segundos enteros.
