...
Cuando es llamada, solicita que el bucle de eventos de Node.js *no* se detenga mientras que el `Timeout` se encuentre activo. Llamar múltiples veces a `timeout.ref()` no tendrá ningún efecto.
