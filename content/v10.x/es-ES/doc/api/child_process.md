...
* `opciones` {Object}
  ...
  * `maxBuffer` {number} La mayor cantidad de datos en bytes permitidos en stdout o stderr. If exceeded, the child process is terminated and any output is truncated. See caveat at [`maxBuffer` and Unicode][]. **Predeterminado:** `200 * 1024`.
...
7. `null`, `undefined` - Utiliza el valor por defecto. For stdio fds 0, 1, and 2 (in other words, stdin, stdout, and stderr) a pipe is created. For fd 3 and up, the default is `'ignore'`.
...
*Vale la pena destacar que cuando se establece un canal IPC entre los procesos principal y secundario, y el secundario es un procesos Node.js, este último se lanza con el canal IPC desreferenciado (usando `unref()`) hasta que regsitra un event handled para el evento [`'disconnect'`][] o el evento [`'message'`][]. Esto permite que el proceso secundario se cierre normalmente sin que se mantenga el proceso abierto por el canal IPC abierto.*
