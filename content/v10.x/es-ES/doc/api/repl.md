...
* `options` {Objeto|string} 
  ...
  * `eval` {Función} La función que será usada al evaluar cada línea de entrada dada. **Default:** an async wrapper for the JavaScript `eval()` function. An `eval` function can error with `repl.Recoverable` to indicate the input was incomplete and prompt for additional lines.
