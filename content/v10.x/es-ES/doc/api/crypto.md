...
The `crypto` module provides the `Certificate` class for working with SPKAC data. The most common usage is handling output generated by the HTML5 `<keygen>` element. Node.js usa la [implementación de SPKAC de OpenSSL](https://www.openssl.org/docs/man1.1.0/apps/openssl-spkac.html) internamente.
...
Calcula el resúmen HMAC de todos los datos pasados usando [`hmac.update()`][]. If `encoding` is provided a string is returned; otherwise a [`Buffer`][] is returned;
...
El `algorithm` es dependiente de OpenSSL, un ejemplo sería `'aes192'`, etc. On recent OpenSSL releases, `openssl list -cipher-algorithms` (`openssl list-cipher-algorithms` for older versions of OpenSSL) will display the available cipher algorithms.
...
The `algorithm` is dependent on the available algorithms supported by the version of OpenSSL on the platform. Los ejemplos son `'sha256'`, `'sha512'`, etc. On recent releases of OpenSSL, `openssl list -digest-algorithms` (`openssl list-message-digest-algorithms` for older versions of OpenSSL) will display the available digest algorithms.
...
If a `callback` function is provided, the bytes are generated asynchronously and the `callback` function is invoked with two arguments: `err` and `buf`. Si ocurre un error, `err` será un objeto `Error`; de no ser así será `null`. The `buf` argument is a [`Buffer`][] containing the generated bytes.
