...
A `Worker` object contains all public information and method about a worker. В основном процессе это можно получить через `cluster.workers`. In a worker it can be obtained using `cluster.worker`.
