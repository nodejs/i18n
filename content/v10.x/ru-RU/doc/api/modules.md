...
In the Node.js module system, each file is treated as a separate module. Например, рассмотрим файл под названием `foo.js`:
...
Локальные переменные модуля будут приватными, поскольку Node.js оборачивает модуль в функцию (см. [module wrapper](#modules_the_module_wrapper)). In this example, the variable `PI` is private to `circle.js`.
...
Modules are cached after the first time they are loaded. Это означает (помимо прочего), что в результате каждого вызова `require('foo')` был бы возвращен абсолютно тот же самый объект, если бы в результате преобразования указанного в нем имени получался путь к одному и тому же файлу.
...
Node.js has several modules compiled into the binary. Эти модули описаны подробнее в других частях документации.
...
A required module prefixed with `'/'` is an absolute path to the file. Например , в результате выполнения `require('/home/marco/foo.js')` будет загружен файл, расположенный по адресу `/home/marco/foo.js`.
