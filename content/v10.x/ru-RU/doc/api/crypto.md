...
The `crypto` module provides the `Certificate` class for working with SPKAC data. The most common usage is handling output generated by the HTML5 `<keygen>` element. Node.js использует [реализацию OpenSSL SPKAC](https://www.openssl.org/docs/man1.1.0/apps/openssl-spkac.html) для внутреннего пользования.
...
Экземпляры класса `Cipher` используются для шифрования данных. The class can be used in one of two ways:
...
* `buffer` {Buffer}
* `опции` {Object} [`stream.transform` options][] 
...
Метод `cipher.setAutoPadding()` должен вызываться до [`cipher.final()`][].
...
Экземпляры класса `Decipher` используются для дешифровки данных. The class can be used in one of two ways:
...
Класс `Hash` - это утилита для создания хэш-дайджестов данных. It can be used in one of two ways:
...
Метод [`crypto.createHash()`][] используется для создания экземпляров `Hash`. `Hash` objects are not to be created directly using the `new` keyword.
...
The `Hash` object can not be used again after `hash.digest()` method has been called. Множественные вызовы приведут к выводу ошибки.
...
Класс `Hmac` - это утилита для создания криптографических дайджестов HMAC. It can be used in one of two ways:
...
Метод [`crypto.createHmac()`][] используется для создания экземпляров `Hmac`. `Hmac` objects are not to be created directly using the `new` keyword.
...
Вычисляет дайджест HMAC всех данных, переданных с помощью [`hmac.update()`][]. If `encoding` is provided a string is returned; otherwise a [`Buffer`][] is returned;

The `Hmac` object can not be used again after `hmac.digest()` has been called. Множественные вызовы `hmac.digest()` приведут к выводу ошибки.
...
Класс `Sign` - это утилита для генерации подписей. It can be used in one of two ways:
...
Метод [`crypto.createSign()`][] используется для создания экземпляров `Sign`. The argument is the string name of the hash function to use. `Sign` objects are not to be created directly using the `new` keyword.
...
In some cases, a `Sign` instance can also be created by passing in a signature algorithm name, such as 'RSA-SHA256'. This will use the corresponding digest algorithm. This does not work for all signature algorithms, such as 'ecdsa-with-SHA256'. Вместо этого используйте названия дайджеста.
...
The `Sign` object can not be again used after `sign.sign()` method has been called. Множественные вызовы `sign.sign()` приведут к выводу ошибки.
...
Класс `Verify` - это утилита для подтверждения подписей. It can be used in one of two ways:
...
Проверяет предоставленные данные при помощи заданных `object` и `signature`. The `object` argument can be either a string containing a PEM encoded object, which can be an RSA public key, a DSA public key, or an X.509 certificate, or an object with one or more of the following properties:
...
* Возвращает: {tls.SecureContext}

The `crypto.createCredentials()` method is a deprecated function for creating and returning a `tls.SecureContext`. Он не должен использоваться. Replace it with [`tls.createSecureContext()`][] which has the exact same arguments and return value.
...
The `algorithm` is dependent on the available algorithms supported by the version of OpenSSL on the platform. Например, `'sha256'`, `'sha512'` и т.д. On recent releases of OpenSSL, `openssl list -digest-algorithms` (`openssl list-message-digest-algorithms` for older versions of OpenSSL) will display the available digest algorithms.
...
Создает и возвращает объект `Sign`, который использует заданный `algorithm`. Use [`crypto.getHashes()`][] to obtain an array of names of the available signing algorithms. Optional `options` argument controls the `stream.Writable` behavior.
...
Создает и возвращает объект `Verify`, который использует заданный алгоритм. Use [`crypto.getHashes()`][] to obtain an array of names of the available signing algorithms. Optional `options` argument controls the `stream.Writable` behavior.
...
Аргумент `iterations` должен быть максимально возможным числом. The higher the number of iterations, the more secure the derived key will be, but will take a longer amount of time to complete.
...
Генерирует криптографически сильные псевдослучайные данные. The `size` argument is a number indicating the number of bytes to generate.
...
The `crypto.randomBytes()` method will not complete until there is sufficient entropy available. Обычно это занимает не более нескольких миллисекунд. The only time when generating the random bytes may conceivably block for a longer period of time is right after boot, when the whole system is still low on entropy.
...
Опциональный аргумент `flags` по умолчанию использует `ENGINE_METHOD_ALL`. The `flags` is a bit field taking one of or a mix of the following flags (defined in `crypto.constants`):
...
Было упрощено использование `ECDH` с нединамически сгенерированными парами ключей. Now, [`ecdh.setPrivateKey()`][] can be called with a preselected private key and the associated public point (key) will be computed and stored in the object. Это позволяет коду хранить и предоставлять только закрытую часть пары ключей EC. [`ecdh.setPrivateKey()`][] now also validates that the private key is valid for the selected curve.
