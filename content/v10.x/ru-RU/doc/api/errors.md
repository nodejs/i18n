...
Подкласс `Ошибки` указывает на ошибку реализации. For details, see [`Class: assert.AssertionError`][].
...
Подкласс `Error`, который указывает, что предоставленный аргумент имеет недопустимый тип. For example, passing a function to a parameter which expects a string would be considered a `TypeError`.
