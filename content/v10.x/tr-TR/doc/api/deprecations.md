...
The [`SlowBuffer`][] class is deprecated. Lütfen bunun yerine [`Buffer.allocUnsafeSlow(size)`][] kullanın.
...
The [`fs.exists(path, callback)`][] API is deprecated. Lütfen bunun yerine [`fs.stat()`][] or [`fs.access()`][] kullanın.
...
The [`punycode`][] module is deprecated. Lütfen bunun yerine bir kullanıcı alternatifi kullanın.
...
The `NODE_REPL_HISTORY_FILE` environment variable was removed. Lütfen bunun yerine `NODE_REPL_HISTORY` kullanın.
...
The [`tls.CryptoStream`][] class was removed. Lütfen bunun yerine [`tls.TLSSocket`][] kullanın.
...
The [`tls.SecurePair`][] class is deprecated. Lütfen bunun yerine [`tls.TLSSocket`][] kullanın.
...
The [`util.isArray()`][] API is deprecated. Lütfen bunun yerine `Array.isArray()` kullanın.
...
The [`util.isBuffer()`][] API is deprecated. Lütfen bunun yerine [`Buffer.isBuffer()`][] kullanın.
...
The `repl` module's `REPL_MODE_MAGIC` constant, used for `replMode` option, has been removed. Its behavior has been functionally identical to that of `REPL_MODE_SLOPPY` since Node.js 6.0.0, when V8 5.0 was imported. Lütfen bunun yerine `REPL_MODE_SLOPPY` kullanın.
...
`timers.enroll()` is deprecated. Lütfen resmi olarak belgelenenleri yerine [`setTimeout()`][] veya [`setInterval()`][] kullanın.
...
`timers.unenroll()` is deprecated. Lütfen resmi olarak belgelenenleri yerine [`clearTimeout()`][] veya [`clearInterval()`][] kullanın.
