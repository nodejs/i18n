...
* [libuv](https://github.com/libuv/libuv): Node.js olay döngüsünün, çalışan iş parçacıklarının ve platformun tüm asenkron davranışlarının yerine getirilmesini sağlayan C kitaplığıdır. Ayrıca, tüm ana işletim sistemlerinde dosya sistemi, soketler, zamanlayıcılar ve sistem olayları ile etkileşimde bulunma gibi birçok ortak sistem görevine POSIX benzeri erişimi kolaylaştıran, çapraz platformlu bir soyutlama kitaplığı işlevi görür. libuv ayrıca standart olay döngüsünün ötesine geçmesi gereken daha karmaşık asenkron Eklentilere güç vermek için kullanılabilecek bir pthreads-like iş parçası soyutlaması da sağlar. Addon authors are encouraged to think about how to avoid blocking the event loop with I/O or other time-intensive tasks by off-loading work via libuv to non-blocking system operations, worker threads or a custom use of libuv's threads.
