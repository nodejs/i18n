...
При успешном выполнении API возвращает `napi_ok`. If a non-detachable `ArrayBuffer` is passed in it returns `napi_detachable_arraybuffer_expected`.
...
N-API предоставляет набор API для получения и установки свойств объектов JavaScript. Some of these types are documented under [Section 7](https://tc39.github.io/ecma262/#sec-abstract-operations) of the [ECMAScript Language Specification](https://tc39.github.io/ecma262/).
...
* `getter`: Функция для вызова, когда выполняется доступ get свойства. Если это передается, установите `value` и `method` на `NULL` (поскольку эти элементы не будут использоваться). The given function is called implicitly by the runtime when the property is accessed from JavaScript code (or if a get on the property is performed using a N-API call).
* `setter`: Функция для вызова, когда выполняется доступ set свойства. Если это передается, установите `value` и `method` на `NULL` (поскольку эти элементы не будут использоваться). The given function is called implicitly by the runtime when the property is set from JavaScript code (or if a set on the property is performed using a N-API call).
...
* `attributes`: Атрибуты, связанные с конкретным свойством. See [`napi_property_attributes`][].
...
* `[out] result`: `napi_value`, представляющее массив значений JavaScript, которые представляют имена свойств объекта. [`napi_get_array_length`][] and [`napi_get_element`][] can be used to iterate over `result`.
...
Этот метод позволяет вызывать объект функции JavaScript из нативного дополнения. This is the primary mechanism of calling back *from* the add-on's native code *into* JavaScript. Для особого случая вызова в JavaScript после асинхронной операции см. [`napi_make_callback`][].
