...
* [`Buffer.alloc(size[, fill[, encoding]])`][`Buffer.alloc()`] returns a new initialized `Buffer` of the specified size. This method is slower than [`Buffer.allocUnsafe(size)`][`Buffer.allocUnsafe()`] but guarantees that newly created `Buffer` instances never contain old data that is potentially sensitive. Если `размер` не является числом, появится `TypeError`.
...
When calling [`Buffer.allocUnsafe()`][] and [`Buffer.allocUnsafeSlow()`][], the segment of allocated memory is *uninitialized* (it is not zeroed-out). При том, что этот дизайн делает выделение памяти довольно быстрым, выделенный сегмент памяти может содержать старые данные, которые являются потенциально незащищенными. Using a `Buffer` created by [`Buffer.allocUnsafe()`][] without *completely* overwriting the memory can allow this old data to be leaked when the `Buffer` memory is read.
...
* `'ascii'`: For 7-bit ASCII data only. Этот метод кодирования очень быстрый, и устранит старший бит, если он установлен.

* `'utf8'`: Multibyte encoded Unicode characters. Многие веб-страницы и другие форматы документов используют UTF-8.

* `'utf16le'`: 2 or 4 bytes, little-endian encoded Unicode characters. Поддерживаются суррогатные пары (U+10000 to U+10FFFF).
...
* `'hex'`: Encode each byte as two hexadecimal characters. Data truncation may occur for unsanitized input. Например:
...
2. Память объекта `Buffer` интерпретируется как массив определенных элементов, а не как байтовый массив целевого типа. That is, `new Uint32Array(Buffer.from([1, 2, 3, 4]))` creates a 4-element [`Uint32Array`][] with elements `[1, 2, 3, 4]`, not a [`Uint32Array`][] with a single element `[0x1020304]` or `[0x4030201]`.
...
* `length` {integer} Количество байтов для отображения. **Default:** `arrayBuffer.byteLength - byteOffset`.
...
Выделяется новый `Buffer` на `size` байтов. If `size` is larger than [`buffer.constants.MAX_LENGTH`][] or smaller than 0, [`ERR_INVALID_OPT_VALUE`][] is thrown. `Buffer` с нулевой длиной создается, если `размер` равен 0.
...
Выделяется новый `Buffer` на `size` байтов. If `fill` is `undefined`, the `Buffer` will be *zero-filled*.
...
If `size` is larger than [`buffer.constants.MAX_LENGTH`][] or smaller than 0, [`ERR_INVALID_OPT_VALUE`][] is thrown. `Buffer` с нулевой длиной создается, если `размер` равен 0.
...
* `byteOffset` {integer} Где начать поиск в `buf`. If negative, then offset is calculated from the end of `buf`. **Default:** `0`.
...
* `value` {string|Buffer|Uint8Array|integer} What to search for.
* `byteOffset` {integer} Где начать поиск в `buf`. If negative, then offset is calculated from the end of `buf`. **Default:** `buf.length - 1`.
* `encoding` {string} If `value` is a string, this is the encoding used to determine the binary representation of the string that will be searched for in `buf`. **Default:** `'utf8'`.
...
Если `byteOffset` не является числом, он будет приведен к числу. Любые аргументы, которые приводят к `NaN`, такие как `{}`, или `undefined`, будут исследовать весь буфер. This behavior matches [`String#lastIndexOf()`][].
...
Возвращает объем памяти, выделенный для `buf` в байтах. This does not necessarily reflect the amount of "usable" data within `buf`.
...
Хотя свойство `length` не является неизменным, изменение значения `length` может привести к неопределенному и противоречивому поведению. Applications that wish to modify the length of a `Buffer` should therefore treat `length` as read-only and use [`buf.slice()`][] to create a new `Buffer`.
...
* `offset` {integer} Количество байтов, которое игнорируется перед началом чтения. Must satisfy: `0 <= offset <= buf.length - 8`. **Default:** `0`.
* Returns: {bigint}
...
* `offset` {integer} Количество байтов, которое игнорируется перед началом чтения. Must satisfy `0 <= offset <= buf.length - 4`. **Default:** `0`.
* Возвращает: {number}
...
* `offset` {integer} Количество байтов, которое игнорируется перед началом чтения. Must satisfy `0 <= offset <= buf.length - 1`. **Default:** `0`.
* Возвращает: {integer}
...
* `offset` {integer} Количество байтов, которое игнорируется перед началом чтения. Must satisfy `0 <= offset <= buf.length - 2`. **Default:** `0`.
* Возвращает: {integer}
...
JavaScript cannot encode 64-bit integers. Этот метод предназначен для работы с 64-битными числами типа float.
...
Декодирует `buf` в строку в соответствии с кодировкой символов, указанной в `encoding`. `start` и `end` могут быть переданы для декодирования только подмножества `buf`. If a byte sequence in the input is not valid in the given `encoding` then it is replaced with the replacement character `U+FFFD`.
...
* `offset` {integer} Количество байтов, которое нужно пропустить перед началом записи. Must satisfy: `0 <= offset <= buf.length - 8`. **Default:** `0`.
...
Записывает `value` в `buf` с заданным `offset` с указанным endian форматом (`writeDoubleBE()` записывает big endian, `writeDoubleLE()` записывает little endian). `value` *should* be a valid 64-bit double. Поведение не определено, когда `value` отличается от 64-битного числа двойной точности.
...
* `value` {number} Количество для записи в `buf`.
* `offset` {integer} Количество байтов, которое нужно пропустить перед началом записи. Must satisfy `0 <= offset <= buf.length - 4`. **Default:** `0`.
* Возвращает: {integer} `offset` плюс количество записанных байтов.

Записывает `value` в `buf` с заданным `offset` с указанным endian форматом (`writeFloatBE()` записывает big endian, `writeFloatLE()` записывает little endian). `value` *should* be a valid 32-bit float. Поведение не определено, когда `value` отличается от 32-битного числа типа float.
...
* `offset` {integer} Количество байтов, которое нужно пропустить перед началом записи. Must satisfy `0 <= offset <= buf.length - 1`. **Default:** `0`.
* Возвращает: {integer} `offset` плюс количество записанных байтов.

Записывает `value` в `buf` с указанным `offset`. `value` *should* be a valid signed 8-bit integer. Поведение не определено, когда `value` отличается от 8-битного целого знакового числа.
...
* `value` {integer} Количество для записи в `buf`.
* `offset` {integer} Количество байтов, которое нужно пропустить перед началом записи. Must satisfy `0 <= offset <= buf.length - 2`. **Default:** `0`.
* Возвращает: {integer} `offset` плюс количество записанных байтов.

Записывает `value` в `buf` с заданным `offset` с указанным endian форматом (`writeInt16BE()` записывает big endian, `writeInt16LE()` записывает little endian). `value` *should* be a valid signed 16-bit integer. Behavior is undefined when `value` is anything other than a signed 16-bit integer.
...
Записывает `value` в `buf` с заданным `offset` с указанным endian форматом (`writeInt32BE()` записывает big endian, `writeInt32LE()` записывает little endian). `value` *should* be a valid signed 32-bit integer. Behavior is undefined when `value` is anything other than a signed 32-bit integer.
...
Записывает `value` в `buf` с указанным `offset`. `value` *should* be a valid unsigned 8-bit integer. Поведение не определено, когда `value` отличается от 8-битного целого числа без знака.
...
Возвращает максимальное количество байтов, которые будут возвращены при вызове `buf.inspect()`. Это может быть переопределено пользовательскими модулями. See [`util.inspect()`][] for more details on `buf.inspect()` behavior.
