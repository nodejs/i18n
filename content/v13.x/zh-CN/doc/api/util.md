...
不同的 Node.js 构建配置支持不同的编码集。 (see [Internationalization](intl.html))
...
* `options` {Object}
  * `fatal` {boolean} `true` 如果解码出现的错误是致命的。 This option is not supported when ICU is disabled (see [Internationalization](intl.html)). **Default:** `false`.
