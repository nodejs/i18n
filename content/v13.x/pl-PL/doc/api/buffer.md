...
* `'hex'`: Encode each byte as two hexadecimal characters. Data truncation may occur for unsanitized input. Na przykład:
...
* `length` {integer} Liczba bajtów do pokazania. **Default:** `arrayBuffer.byteLength - byteOffset`.
