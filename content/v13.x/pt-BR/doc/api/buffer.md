...
* `'hex'`: Encode each byte as two hexadecimal characters. Data truncation may occur for unsanitized input. Por exemplo:
...
* `length` {integer} Número de bytes para expor. **Default:** `arrayBuffer.byteLength - byteOffset`.
