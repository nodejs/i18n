...
In the case of Promises, the `resource` object will have an `isChainedPromise` property, set to `true` if the promise has a parent promise, and `false` otherwise. For example, in the case of `b = a.then(handler)`, `a` is considered a parent `Promise` of `b`. Aqui, `b` é considerado uma promessa encadeada.
...
Promise contexts may not get precise `executionAsyncIds` by default. Veja a seção em [rastreamento de execução de promessas](#async_hooks_promise_execution_tracking).
...
Chamar todos os hooks `destroy`. Isto só deverá apenas ser chamado uma vez. An error will be thrown if it is called more than once. This **must** be manually called. If the resource is left to be collected by the GC then the `destroy` hooks will never be called.
