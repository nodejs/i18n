...
* `port` {number} Порт для прослушивания соединений инспектора. Необязательный. **Default:** what was specified on the CLI.
* `host` {string} Хост для прослушивания соединений инспектора. Необязательный. **Default:** what was specified on the CLI.
* `wait` {boolean} Блокировать, пока клиент не подключился. Необязательный. **Default:** `false`.
