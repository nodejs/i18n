...
O módulo `v8` expõe APIs que são específicas à versão do [V8](https://developers.google.com/v8/) incluída no binário do Node.js. Ele pode ser acessado usando:

```js
const v8 = require('v8');
```

As APIs e a implementação estão sujeitas a alterações a qualquer momento.
