...
* `options` {Object}
  ...
  * `key` {string|string[]|Buffer|Buffer[]|Object[]} Private keys in PEM format. PEM permite la opción de encriptar claves privadas. Encrypted keys will be decrypted with `options.passphrase`. Multiple keys using different algorithms can be provided either as an array of unencrypted key strings or buffers, or an array of objects in the form `{pem: <string|buffer>[, passphrase: <string>]}`. La forma del objeto solo puede ocurrir en un array. `object.passphrase` es opcional. Encrypted keys will be decrypted with `object.passphrase` if provided, or `options.passphrase` if it is not.
