...
* [libuv](https://github.com/libuv/libuv): Библиотека C, которая реализует цикл событий Node.js, его рабочие потоки и все асинхронные поведения платформы. Она также обеспечивает кроссплатформенную библиотеку абстракций, предоставляет простой доступ типа POSIX через все главные операционные системы ко многим основным системным задачам, таким как взаимодействие с файловой системой, сокеты, таймеры и системные события. libuv also provides a pthreads-like threading abstraction that may be used to power more sophisticated asynchronous Addons that need to move beyond the standard event loop. Addon authors are encouraged to think about how to avoid blocking the event loop with I/O or other time-intensive tasks by off-loading work via libuv to non-blocking system operations, worker threads or a custom use of libuv's threads.
