...
Si una función `callback` es dada, los bytes son geberados asincrónicamente y, la función `callback` es invocada con dos argumentos: `err` y `buf`. Si un error ocurre, `err` sera un objeto de Error, de lo contrario sera nulo. The `buf` argument is a [`Buffer`][] containing the generated bytes.
